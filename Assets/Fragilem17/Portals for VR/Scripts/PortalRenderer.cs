...
namespace Fragilem17.MirrorsAndPortals
{
    ...
    public class PortalRenderer : MonoBehaviour
    {
		...
		[Tooltip("Rendertextures would sometimes be flipped. Check to unflip them when needed.")]
        ...
        [Tooltip("When enabled the portal will be rendered in the sceneview as well as the game view. If you can't turn on this checkbox, check out the warning on top of the component.")]
            ...
            if (renderCamera.cameraType != CameraType.SceneView)
            {
                if (_closestPortal != null && _clippingPlaneLeft)
                {
                    ...
                    if (_clippingPlaneMaterial && (leftMeshDrawn || rightMeshDrawn))
                    {
						...
						if (_clippingPlaneMaterial.HasProperty("_flipY"))
						{
							//Debug.Log("depth: " + depth + " " + gameObject.name);
							if (FlipInBuild)
							{
								_clippingPlaneMaterial.SetInt("_flipY", 1);
							}
							else if (Application.isEditor && CurrentRenderCamera.cameraType == CameraType.SceneView && FlipInSceneView)
							{
								_clippingPlaneMaterial.SetInt("_flipY", 1);
							}
							else if (Application.isEditor && CurrentRenderCamera.cameraType == CameraType.Game && FlipInGameView)
							{
								_clippingPlaneMaterial.SetInt("_flipY", 1);
							}
							else
							{
								_clippingPlaneMaterial.SetInt("_flipY", 0);
							}
						}
                ...
                else
                {
                    ...
                    if (li.Count > 0)
                    {
                        foreach (CameraPortalMatrices cm in li)
                        {
                            if (cm != matrices)
                            {
                                PooledPortalTexture p = _pooledTextures.Find(ptex => ptex.renderCam == reflectionCamera && ptex.matrices.mirrorSurface == cm.mirrorSurface
		...
		private void GetFreeTexture(out PooledPortalTexture textureOut, Camera.StereoscopicEye eye = Camera.StereoscopicEye.Left, Camera cam = null)
        {
            if (cam == null) {
                cam = _currentRenderCamera;
			}
            PooledPortalTexture tex = _pooledTextures.Find(tex => !tex.fullLock && !tex.liteLock && tex.eye == eye && tex.renderCam == cam);
            if (tex == null)
			{
                ...
                tex.texture.name = "_Tex" + gameObject.name + "_" + cam.name + "_" + _pooledTextures.Count;
